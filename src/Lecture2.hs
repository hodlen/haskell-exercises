...
removeAt i list =
  if i < 0
    then (Nothing, list)
      ...
      let picked = case drop i list of
            [] -> Nothing
            x : _ -> Just x
       in (picked, take i list ++ drop (i + 1) list)
...
dropSpaces str =
  let dropPreSpaces :: String -> String
      dropPreSpaces s = case s of
        ' ' : s' -> dropPreSpaces s'
        s' -> s'
      dropPostSpaces :: String -> String
      dropPostSpaces s = case s of
        "" -> ""
        ' ' : _ -> ""
        c : s' -> c : dropPostSpaces s'
   in dropPostSpaces (dropPreSpaces str)
...
ceilingDiv m n = case divMod m n of
  (frac, 0) -> frac
  (frac, _) -> frac + 1

dragonFight :: Knight -> Dragon -> Int
dragonFight knight dragon =
  let dragonCanSurvive = ceilingDiv (dragonHealth dragon) (knightAttack knight)
      knightCanSurvive = 10 * ceilingDiv (knightHealth knight) (dragonAttack dragon)
      knightCanEndure = knightEndurance knight
   in dragonCanSurvive + knightCanSurvive + knightCanEndure
...
isIncreasing list = case list of
  [] -> True
  [_] -> True
  x : y : rest -> x <= y && isIncreasing (y : rest)
...
-- [1,2,3]
divideTwoFold :: [a] -> ([a], [a])
divideTwoFold list = (fstHalf, sndHalf)
  where
    split = div (length list) 2
    fstHalf = take split list
    sndHalf = drop split list
...
mergeSort list = case list of
  ...
  _ -> case divideTwoFold list of
    (hl, tl) -> merge (mergeSort hl) (mergeSort tl)
...
-- sortExps ::(Expr, Expr) -> (Expr, Expr) -- keep only atomic value on left, add only on
-- sortExps (Var x, expr) = (Var x, expr)
-- sortExps (Lit l, expr) = case expr of
--   Lit l' -> (Lit l, Lit l')
--   Var x -> (Var x, Lit l)
--   Add

splitVars :: Expr -> ([String], Int)
splitVars expr = case expr of
  Lit l -> ([], l)
  Var x -> ([x], 0)
  Add exp1 exp2 -> (fst res1 ++ fst res2, snd res1 + snd res2)
    where
      res1 = splitVars exp1
      res2 = splitVars exp2

constantFolding :: Expr -> Expr
constantFolding expr = case splitVars expr of
  ([], n) -> Lit n
  ([x], 0) -> Var x
  ([x], n) -> Add (Var x) (Lit n)
  (x0 : x1 : xs, n) ->
    let addedVars = foldr (\x e -> Add e (Var x)) (Add (Var x0) (Var x1)) xs
     in case n of
          0 -> addedVars
          _ -> Add addedVars (Lit n)
