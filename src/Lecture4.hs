...
splitOn str char =
  let go :: [String] -> String -> String -> [String]
      go acc acceptBuf toReadBuf = case toReadBuf of
        [] -> case acceptBuf of
          [] -> acc
          _ -> acc ++ [acceptBuf]
        c : cs -> if c == char then go (acc ++ [acceptBuf]) "" cs else go acc (acceptBuf ++ [c]) cs
   in go [] "" str
...
parseRow raw = case splitOn raw ',' of
  [rawProduct, rawTradeType, rawCost] ->
    justList rawProduct
      >>= ( \someProduct ->
              readMaybe rawTradeType >>= \tradeType ->
                readMaybe rawCost >>= \cost ->
                  if cost >= 0 then Just Row {rowProduct = someProduct, rowTradeType = tradeType, rowCost = cost} else Nothing
          )
  _ -> Nothing
...
instance Semigroup MaxLen where
  ma <> mb = MaxLen {unMaxLen = longerString (unMaxLen ma) (unMaxLen mb)}
...
instance Semigroup Stats where
  sa <> sb =
    Stats
      { statsTotalPositions = statsTotalPositions sa <> statsTotalPositions sb,
        statsTotalSum = statsTotalSum sa <> statsTotalSum sb,
        statsAbsoluteMax = statsAbsoluteMax sa <> statsAbsoluteMax sb,
        statsAbsoluteMin = statsAbsoluteMin sa <> statsAbsoluteMin sb,
        statsSellMax = statsSellMax sa <> statsSellMax sb,
        statsSellMin = statsSellMin sa <> statsSellMin sb,
        statsBuyMax = statsBuyMax sa <> statsBuyMax sb,
        statsBuyMin = statsBuyMin sa <> statsBuyMin sb,
        statsLongest = statsLongest sa <> statsLongest sb
      }
    ...
    Sell ->
      Stats
        { statsTotalPositions = Sum 1,
          ...
          statsBuyMax = Nothing,
          statsBuyMin = Nothing,
          statsLongest = MaxLen {unMaxLen = productName}
        }
    Buy ->
      Stats
        { statsTotalPositions = Sum 1,
...
combineRows (firstRow :| rows) = sconcat (rowToStats firstRow :| map rowToStats rows)
...
showTwoColumnLine :: String -> String -> String
showTwoColumnLine title value = title ++ replicate (23 - length title) ' ' ++ ": " ++ value ++ "\n"

showMaybeValue :: Show m => Maybe m -> String
showMaybeValue = maybe "no value" show

displayStats :: Stats -> String
displayStats
  Stats
    { statsTotalPositions = totalPositions,
      statsTotalSum = totalSum,
      statsAbsoluteMax = absoluteMax,
      statsAbsoluteMin = absoluteMin,
    ...
    showTwoColumnLine "Total positions" ((show . getSum) totalPositions)
      ++ showTwoColumnLine "Total final balance" ((show . getSum) totalSum)
      ++ showTwoColumnLine "Biggest absolute cost" ((show . getMax) absoluteMax)
      ++ showTwoColumnLine "Smallest absolute cost" ((show . getMin) absoluteMin)
      ++ showTwoColumnLine "Max earning" (showMaybeValue (sellMax >>= Just . getMax))
      ++ showTwoColumnLine "Min earning" (showMaybeValue (sellMin >>= Just . getMin))
      ++ showTwoColumnLine "Max spending" (showMaybeValue (buyMax >>= Just . getMax))
      ++ showTwoColumnLine "Min spending" (showMaybeValue (buyMin >>= Just . getMin))
      ++ showTwoColumnLine "Longest product name" (unMaxLen longest)
...
calculateStats fileStr =
  fromMaybe
    "the file doesn't have any products"
    ( justList (splitOn fileStr '\n')
        >>= (justList . mapMaybe parseRow)
        >>= (\rows -> return (displayStats (combineRows (head rows :| tail rows))))
    )
