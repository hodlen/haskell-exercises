...
parseRow row = case splitOn ',' row of
  [x0, x1, x2] -> do
    tradeType <- readMaybe x1
    cost <- readMaybe x2
    positiveCost <- if cost < 0 then Nothing else Just cost
    product' <- nonEmpty x0
    Just Row {rowProduct = toList product', rowTradeType = tradeType, rowCost = positiveCost}
  _ -> Nothing
...
rowToStats (Row product' tradeType cost) =
  Stats
    { statsTotalPositions = 1,
      ...
      statsSellMax = max' sellCost,
      statsSellMin = min' sellCost,
      statsBuyMax = max' buyCost,
      statsBuyMin = min' buyCost,
      statsLongest = MaxLen product'
    }
  where
    (buyCost, sellCost) = if tradeType == Buy then (Just cost, Nothing) else (Nothing, Just cost)
    max' = fmap Max
    min' = fmap Min
...
displayStats s =
  "Total positions        : "
    ...
    ++ "\n"
    ++ "Max earning            : "
    ++ showMaybe (fmap getMax (statsSellMax s))
    ++ "\n"
    ++ "Min earning            : "
    ++ showMaybe (fmap getMin (statsSellMin s))
    ++ "\n"
    ++ "Max spending           : "
    ++ showMaybe (fmap getMax (statsBuyMax s))
    ++ "\n"
    ++ "Min spending           : "
    ++ showMaybe (fmap getMin (statsBuyMin s))
    ++ "\n"
    ++ "Longest product name   : "
    ++ unMaxLen (statsLongest s)
  where
    showMaybe :: (Show a) => Maybe a -> String
    showMaybe = maybe "no value" show
...
parseRows :: String -> Maybe (NonEmpty Row)
parseRows content = case lines content of
  [] -> Nothing
  rowStrings -> nonEmpty (mapMaybe parseRow rowStrings)

calculateStats :: String -> String
calculateStats content = maybe "the file doesn't have any products" (displayStats . combineRows) (parseRows content)
