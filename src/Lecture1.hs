...
strSum str = sum nums
    where nums = map (\s -> read s :: Integer) (words str)
...
lowerAndGreater n list = show n ++ " is greater than " ++ show nLower ++ " elements and lower than " ++ show nGreater ++ " elements"
    where (nGreater, nLower) = lowerAndGreaterCount n list 0 0

lowerAndGreaterCount :: Integer -> [Integer] -> Integer -> Integer -> (Integer, Integer)
lowerAndGreaterCount n list nGreater nLower = case list of
    [] -> (nGreater, nLower)
    (x:xs) -> lowerAndGreaterCount n xs (if x > n then succ nGreater else nGreater) (if x < n then succ nLower else nLower)
